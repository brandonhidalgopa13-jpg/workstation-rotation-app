        ...
        lifecycleScope.launch {
            try {
                ...
                if (loginResult.isSuccess) {
                    ...
                    handleFailedLogin(username, loginResult.errorMessage)
                }
            ...
            callback = object : BiometricAuthManager.AuthCallback {
                override fun onAuthenticationSucceeded() {
                    lifecycleScope.launch {
                        ...
                        val userRole = getUserRole(savedUsername)
                        val deviceId = getDeviceId()
                        
                        when (val sessionResult = sessionManager.startSession(savedUsername, userRole, deviceId)) {
                            is SessionResult.Success -> {
                                saveSessionToken(sessionResult.session.token)
